...
use protocol::common::{Date, Nullable, WebElement, FrameId, LocatorStrategy};
...
impl Parameters for NewSessionParameters {
    fn from_json(body: &Json) -> WebDriverResult<NewSessionParameters> {
        ...
        let desired_capabilities = if let Some(capabilities) = data.get("desiredCapabilities") {
            try_opt!(capabilities.as_object(),
                     ErrorStatus::InvalidArgument,
                     "'desiredCapabilities' parameter is not an object")
                .clone()
        } else {
            BTreeMap::new()
        };

        let required_capabilities = if let Some(capabilities) = data.get("requiredCapabilities") {
            try_opt!(capabilities.as_object(),
                     ErrorStatus::InvalidArgument,
...
impl Parameters for GetParameters {
    fn from_json(body: &Json) -> WebDriverResult<GetParameters> {
        let data = try_opt!(body.as_object(),
                            ErrorStatus::UnknownError,
                            "Message body was not an object");
        let url = try_opt!(try_opt!(data.get("url"),
                                    ErrorStatus::InvalidArgument,
                                    "Missing 'url' parameter")
                               .as_string(),
                           ErrorStatus::InvalidArgument,
                           "'url' not a string");
        Ok(GetParameters { url: url.to_string() })
    }
}
...
impl Parameters for TimeoutsParameters {
    fn from_json(body: &Json) -> WebDriverResult<TimeoutsParameters> {
        let data = try_opt!(body.as_object(),
                            ErrorStatus::UnknownError,
                            "Message body was not an object");
        let type_ = try_opt!(try_opt!(data.get("type"),
                                      ErrorStatus::InvalidArgument,
        ...
        let ms = try_opt!(try_opt!(data.get("ms"),
                                   ErrorStatus::InvalidArgument,
                                   "Missing 'ms' parameter")
                              .as_f64(),
                          ErrorStatus::InvalidArgument,
                          "'ms' not a float");
...
impl Parameters for WindowSizeParameters {
    fn from_json(body: &Json) -> WebDriverResult<WindowSizeParameters> {
        let data = try_opt!(body.as_object(),
                            ErrorStatus::UnknownError,
                            "Message body was not an object");
        let height = try_opt!(try_opt!(data.get("height"),
                                       ErrorStatus::InvalidArgument,
                                       "Missing 'height' parameter")
                                  .as_u64(),
                              ErrorStatus::InvalidArgument,
                              "'height' is not a positive integer");
        let width = try_opt!(try_opt!(data.get("width"),
                                      ErrorStatus::InvalidArgument,
...
impl Parameters for SwitchToWindowParameters {
    fn from_json(body: &Json) -> WebDriverResult<SwitchToWindowParameters> {
        let data = try_opt!(body.as_object(),
                            ErrorStatus::UnknownError,
                            "Message body was not an object");
        let handle = try_opt!(try_opt!(data.get("handle"),
                                       ErrorStatus::InvalidArgument,
                                       "Missing 'handle' parameter")
                                  .as_string(),
                              ErrorStatus::InvalidArgument,
                              "'handle' not a string");
        return Ok(SwitchToWindowParameters { handle: handle.to_string() });
    }
}
...
impl Parameters for LocatorParameters {
    fn from_json(body: &Json) -> WebDriverResult<LocatorParameters> {
        ...
        let using = try!(LocatorStrategy::from_json(try_opt!(data.get("using"),
                                                             ErrorStatus::InvalidArgument,
        ...
        let value = try_opt!(try_opt!(data.get("value"),
                                 ...
                                 .as_string(),
                             ErrorStatus::InvalidArgument,
                             "Could not convert using to string")
            .to_string();
...
impl Parameters for SwitchToFrameParameters {
    fn from_json(body: &Json) -> WebDriverResult<SwitchToFrameParameters> {
        ...
        Ok(SwitchToFrameParameters { id: id })
    }
}
...
impl Parameters for SendKeysParameters {
    fn from_json(body: &Json) -> WebDriverResult<SendKeysParameters> {
        ...
        Ok(SendKeysParameters { value: value })
    }
}
...
impl ToJson for SendKeysParameters {
    fn to_json(&self) -> Json {
        let mut data = BTreeMap::new();
        let value_string: Vec<String> = self.value
            .iter()
            .map(|x| x.to_string())
            .collect();
...
impl Parameters for JavascriptCommandParameters {
    fn from_json(body: &Json) -> WebDriverResult<JavascriptCommandParameters> {
        ...
        let args = try!(Nullable::from_json(args_json, |x| {
            Ok((try_opt!(x.as_array(),
                         ErrorStatus::InvalidArgument,
                         "Failed to convert args to Array"))
                .clone())
        }));

        // TODO: Look for WebElements in args?
        let script = try_opt!(try_opt!(data.get("script"),
                                       ErrorStatus::InvalidArgument,
...
impl Parameters for GetCookieParameters {
    fn from_json(body: &Json) -> WebDriverResult<GetCookieParameters> {
        ...
        return Ok(GetCookieParameters { name: name });
    }
}
...
impl Parameters for AddCookieParameters {
    fn from_json(body: &Json) -> WebDriverResult<AddCookieParameters> {
        ...
        let name = try_opt!(try_opt!(data.get("name"),
                                     ErrorStatus::InvalidArgument,
                                     "Missing 'name' parameter")
                                .as_string(),
                            ErrorStatus::InvalidArgument,
        ...
        let path = match data.get("path") {
            Some(path_json) => {
                try!(Nullable::from_json(path_json, |x| {
                    Ok(try_opt!(x.as_string(),
                                ErrorStatus::InvalidArgument,
        ...
        let domain = match data.get("domain") {
            Some(domain_json) => {
                try!(Nullable::from_json(domain_json, |x| {
                    Ok(try_opt!(x.as_string(),
                                ErrorStatus::InvalidArgument,
                                "Failed to convert domain to String")
                        .to_string())
                }))
            }
            None => Nullable::Null,
        ...
        let expiry = match data.get("expiry") {
            Some(expiry_json) => {
                try!(Nullable::from_json(expiry_json, |x| {
...
impl Parameters for TakeScreenshotParameters {
    fn from_json(body: &Json) -> WebDriverResult<TakeScreenshotParameters> {
        ...
        let element = match data.get("element") {
            Some(element_json) => {
                try!(Nullable::from_json(element_json, |x| Ok(try!(WebElement::from_json(x)))))
            }
            None => Nullable::Null,
        };

        return Ok(TakeScreenshotParameters { element: element });
    }
}
...
impl Parameters for ActionsParameters {
    fn from_json(body: &Json) -> WebDriverResult<ActionsParameters> {
        try_opt!(body.as_object(),
                 ErrorStatus::InvalidArgument,
                 "Message body was not an object");
        let actions = try_opt!(try_opt!(body.find("actions"),
                                        ErrorStatus::InvalidArgument,
                                        "No actions parameter found")
                                   .as_array(),
                               ErrorStatus::InvalidArgument,
...
impl Parameters for ActionSequence {
    fn from_json(body: &Json) -> WebDriverResult<ActionSequence> {
        ...
        let actions = match type_name {
            ...
            _ => {
                return Err(WebDriverError::new(ErrorStatus::InvalidArgument, "Invalid action type"))
            }
        };
    ...
    fn to_json(&self) -> Json {
        ...
        let (action_type, actions) = match self.actions {
            ActionsType::Null(ref actions) => {
                ("none", actions.iter().map(|x| x.to_json()).collect::<Vec<Json>>())
            }
            ActionsType::Key(ref actions) => {
                ("key", actions.iter().map(|x| x.to_json()).collect::<Vec<Json>>())
            }
            ActionsType::Pointer(ref parameters, ref actions) => {
                data.insert("parameters".into(), parameters.to_json());
                ("pointer", actions.iter().map(|x| x.to_json()).collect::<Vec<Json>>())
            }
        };
...
impl Parameters for ActionsType {
    fn from_json(body: &Json) -> WebDriverResult<ActionsType> {
        ...
        let actions_type =
            body.find("type").and_then(|x| x.as_string()).expect("Type should be a string");
        let actions_chain = try_opt!(try_opt!(data.get("actions"),
                                              ErrorStatus::InvalidArgument,
                                              "Missing actions parameter")
                                         .as_array(),
                                     ErrorStatus::InvalidArgument,
                                     "Parameter 'actions' was not an array");
        match actions_type {
...
impl Parameters for PointerType {
    fn from_json(body: &Json) -> WebDriverResult<PointerType> {
        match body.as_string() {
            ...
            Some(_) => {
                Err(WebDriverError::new(ErrorStatus::InvalidArgument, "Unsupported pointer type"))
            }
            None => {
...
impl Parameters for PointerActionParameters {
    fn from_json(body: &Json) -> WebDriverResult<PointerActionParameters> {
        ...
        Ok(PointerActionParameters { pointer_type: pointer_type })
    }
}
...
impl ToJson for PointerActionParameters {
    fn to_json(&self) -> Json {
        let mut data = BTreeMap::new();
        data.insert("pointerType".to_owned(), self.pointer_type.to_json());
        Json::Object(data)
    }
}
...
impl Parameters for NullActionItem {
    fn from_json(body: &Json) -> WebDriverResult<NullActionItem> {
        ...
        let type_name = try_opt!(try_opt!(data.get("type"),
                                 ...
                                 "Parameter 'type' was not a string");
        match type_name {
            "pause" => Ok(NullActionItem::General(try!(GeneralAction::from_json(body)))),
            _ => {
                return Err(WebDriverError::new(ErrorStatus::InvalidArgument,
...
impl Parameters for KeyActionItem {
    fn from_json(body: &Json) -> WebDriverResult<KeyActionItem> {
        ...
        let type_name = try_opt!(try_opt!(data.get("type"),
                                 ...
                                 "Parameter 'type' was not a string");
        match type_name {
            "pause" => Ok(KeyActionItem::General(try!(GeneralAction::from_json(body)))),
            _ => Ok(KeyActionItem::Key(try!(KeyAction::from_json(body)))),
        }
    }
}
...
impl ToJson for PauseAction {
    fn to_json(&self) -> Json {
        let mut data = BTreeMap::new();
        data.insert("type".to_owned(), "pause".to_json());
        data.insert("duration".to_owned(), self.duration.to_json());
        Json::Object(data)
    }
}
...
impl Parameters for KeyUpAction {
    fn from_json(body: &Json) -> WebDriverResult<KeyUpAction> {
        let value_str = try_opt!(try_opt!(body.find("value"),
                                          ErrorStatus::InvalidArgument,
                                          "Missing value parameter")
                                     .as_string(),
                                 ErrorStatus::InvalidArgument,
...
impl ToJson for KeyUpAction {
    fn to_json(&self) -> Json {
        let mut data = BTreeMap::new();
        data.insert("type".to_owned(), "keyUp".to_json());
        data.insert("value".to_string(), self.value.to_string().to_json());
        Json::Object(data)
    }
}
...
impl ToJson for KeyDownAction {
    fn to_json(&self) -> Json {
        let mut data = BTreeMap::new();
        data.insert("type".to_owned(), "keyUp".to_json());
        data.insert("value".to_owned(), self.value.to_string().to_json());
        Json::Object(data)
    }
}
...
impl Parameters for PointerAction {
    fn from_json(body: &Json) -> WebDriverResult<PointerAction> {
        match body.find("type").and_then(|x| x.as_string()) {
            ...
            Some("pointerDown") => {
                Ok(PointerAction::Down(try!(PointerDownAction::from_json(body))))
            }
    ...
    fn to_json(&self) -> Json {
        match self {
            ...
            &PointerAction::Cancel => {
                let mut data = BTreeMap::new();
                data.insert("type".to_owned(), "cancel".to_json());
                Json::Object(data)
            }
        }
    }
...
impl Parameters for PointerUpAction {
    fn from_json(body: &Json) -> WebDriverResult<PointerUpAction> {
        let button = try_opt!(try_opt!(body.find("button"),
                                       ErrorStatus::InvalidArgument,
        ...
        Ok(PointerUpAction { button: button })
    }
}
...
impl ToJson for PointerUpAction {
    fn to_json(&self) -> Json {
        let mut data = BTreeMap::new();
        data.insert("type".to_owned(), "pointerUp".to_json());
...
impl Parameters for PointerDownAction {
    fn from_json(body: &Json) -> WebDriverResult<PointerDownAction> {
        ...
        Ok(PointerDownAction { button: button })
    }
}
...
impl ToJson for PointerDownAction {
    fn to_json(&self) -> Json {
        let mut data = BTreeMap::new();
        data.insert("type".to_owned(), "pointerDown".to_json());
        data.insert("button".to_owned(), self.button.to_json());
        Json::Object(data)
    }
}
...
impl ToJson for PointerMoveAction {
    fn to_json(&self) -> Json {
        ...
        if self.element.is_value() {
            data.insert("element".to_owned(), self.element.to_json());
        }
        if self.x.is_value() {
